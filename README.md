...
2. 完成一个任务:**直接在任务文件里把 `- [ ]` 改成 `- [x]` 打勾**。开着 `python study.py watch` 时打勾会自动同步,什么都不用发;没开 watch 就手动跑 `python study.py sync`;也可以不碰文件,跑 `python study.py done <编号> 备注`
...
| `python study.py watch` | 后台监听任务文件,打勾后自动同步(Ctrl+C 停止) |
